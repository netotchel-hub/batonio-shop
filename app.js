// Конфигурация
const CONFIG = {
    API_URL: 'https://script.google.com/macros/s/AKfycbz7XL5RH_8UHCbeKTq_1W3vOqaS4LnvqPDkFFn9OJ9Rdxz-RHb0IDvzixcjDeirbXzhGg/exec'
};
...
async function submitOrder(orderData) {
    try {
        const response = await fetch(CONFIG.API_URL, {
            ...
            body: JSON.stringify({
                action: 'placeOrder',
                telegramUserId: state.telegramUserId || 0,
                ...orderData,
                cart: state.cart,
                totalSum: state.cart.reduce((sum, item) => sum + item.total, 0),
                agreeToTerms: true
            })
        });
...
function applyTheme(color) {
    ...
    document.querySelector('.header').style.background = `linear-gradient(135deg, ${color} 0%, ${adjustColor(color, -50)} 100%)`;
}

function applyCategoryTheme(style) {
    document.querySelector('.header').style.background = `linear-gradient(135deg, ${style.color} 0%, ${style.color2} 100%)`;
}
