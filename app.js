...
async function loadProducts() {
    try {
        ...
        if (data.products && data.products.length > 0) {
            ...
            console.log('Категории товаров:', [...new Set(state.products.map(p => p.category))]);
        } else {
...
// ==================== КАТЕГОРИИ (ПОЛНАЯ ИСПРАВЛЕННАЯ ФУНКЦИЯ) ====================
window.filterProducts = function(category) {
    ...
    if (category === 'все') {
        ...
        console.log('Показаны все товары:', filteredProducts.length);
    } else {
        filteredProducts = state.products.filter(product => {
            ...
            console.log(`Товар: ${product.name}, Категория: "${productCategory}", Ищем: "${category}"`);
            
            // Проверяем совпадение
            const matches = productCategory === category;
            return matches;
        });
        console.log(`Найдено товаров в категории "${category}":`, filteredProducts.length);
    }
    
    // Если нет товаров в категории
    if (filteredProducts.length === 0 && category !== 'все') {
        const container = document.getElementById('productsContainer');
        container.innerHTML = `
            <div style="grid-column: 1 / -1; text-align: center; padding: 40px;">
                <p>Товаров в категории "${category}" пока нет</p>
                <button onclick="filterProducts('все')" style="margin-top: 20px; padding: 10px 20px; background: #E27D60; color: white; border: none; border-radius: 6px; cursor: pointer;">
                    Показать все товары
                </button>
            </div>
        `;
    ...
    products.forEach(product => {
        ...
        card.innerHTML = `
            ...
            <div class="product-info">
                ...
                <p class="product-category" style="font-size: 10px; color: #999; margin-top: 3px;">
...
window.forceOpenShop = function() {
    // Функция для принудительного открытия магазина
    state.isWorkingTime = true;
    hideNonWorkingOverlay();
    console.log('Магазин принудительно открыт');
...
window.forceCloseShop = function() {
    // Функция для принудительного закрытия магазина
    state.isWorkingTime = false;
    showNonWorkingOverlay();
    console.log('Магазин принудительно закрыт');
...
// ==================== УТИЛИТЫ ====================
function testFilter() {
    // Тестовая функция для отладки фильтрации
    console.log('=== ТЕСТ ФИЛЬТРАЦИИ ===');
    console.log('Все товары:', state.products.length);
    console.log('Категории товаров:', [...new Set(state.products.map(p => p.category))]);
    
    // Тестируем фильтрацию
    filterProducts('выпечка');
    setTimeout(() => filterProducts('заморозка'), 1000);
    setTimeout(() => filterProducts('все'), 2000);
}